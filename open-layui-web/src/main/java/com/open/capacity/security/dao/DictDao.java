...
public interface DictDao {
    ...
    @Select("select * from dict t where t.id = #{id}")
    ...
    @Insert("insert into dict(type, k, val, createTime, updateTime) values(#{type}, #{k}, #{val}, now(), now())")
    int save(Dict dict);
    ...
    @Select("select * from dict t where t.type = #{type} and k = #{k}")
